...
export function isMissingOptionalTableError(
  ...
  return missingTable && message.toLowerCase().includes(table.toLowerCase());
}

async function deleteOptionalUserRows(
  db: Db,
  table: string,
  column: string,
  userId: string,
) {
  const result = await (db as any).from(table).delete().eq(column, userId);
  if (isMissingOptionalTableError(result.error, table)) {
...
async function deleteWhereIn(
  ...
  for (const batch of chunks(values)) {
    const { error } = await (db as any).from(table).delete().in(column, batch);
    await throwIfError(error, `Failed to delete ${table}`);
  }
}
...
export async function deleteUserProjects(
  ...
  const chatIds = uniqueStrings(
    ((projectChats.data ?? []) as { id: string | null }[]).map((row) => row.id),
  );
  const reviewIds = uniqueStrings(
